...
export default function LaunchStatus({ activeApplications = [] }) {
  const columns = [
          ...
          {columns.map(col => (
            <div key={col.id} className="flex-1 bg-slate-900/20 border border-white/5 rounded-2xl p-4 flex flex-col">
              ...
              <div className="space-y-4 flex-1">
                {/* Dynamically render ONLY the applications the user selected */}
                {activeApplications.filter(app => app.status === col.id).map(app => (
                  <div key={app.id} className="bg-slate-900/60 border border-white/10 rounded-xl p-4 hover:border-amber-500/30 transition-colors shadow-lg cursor-grab">
                    ...
                    <p className="text-xs font-semibold text-emerald-400 mt-2 mb-3">Target: {app.amount}</p>
                    
                    {/* Direct link out to the actual application portal */}
                    {app.portalUrl && (
                      <a href={app.portalUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold text-blue-400 hover:text-blue-300 block border-t border-white/5 pt-2">
                ...
                {/* Empty State UI */}
                {activeApplications.filter(app => app.status === col.id).length === 0 && (
