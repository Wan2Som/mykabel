 ...
 // 1. Handle Email Sign Up
  const handleEmailSignUp = async (e) => {
    ...
    try {
      ...
      // Save their company name to Firestore Database securely!
      ...
      // Show the success message, then wait 1.5 seconds before routing
      setSuccess("Account created successfully! Booting up ecosystem...");
      setTimeout(() => {
        router.push('/'); 
      }, 1500);

    } catch (err) {
      if(err.code === 'auth/email-already-in-use') setError("Email already exists. Please sign in.");
  ...
  return (
    <div className="font-sans min-h-screen flex items-center justify-center p-4 text-slate-200 relative overflow-hidden" 
      ...
      <div className="glass-card w-full max-w-[450px] p-10 rounded-2xl my-8">
        <div className="text-center mb-8 flex flex-col items-center">
          <svg width="55" height="55" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="mb-4 drop-shadow-[0_0_10px_rgba(245,158,11,0.3)]">
          ...
          <p className="text-sm font-bold text-slate-300 tracking-wide">Build your ecosystem profile.</p>
        </div>
        ...
        {success && <p className="text-emerald-400 text-xs text-center font-bold mb-4 bg-emerald-900/30 py-2 rounded border border-emerald-500/20">{success}</p>}
        
        <button onClick={handleGoogleSignUp} className="w-full flex items-center justify-center gap-3 bg-slate-800/80 border border-slate-600 rounded-xl px-4 py-3.5 text-sm font-bold text-white hover:bg-slate-700 hover:border-amber-500 transition-all focus:outline-none backdrop-blur-sm">
        ...
        <div className="flex items-center my-6">
          ...
          <span className="px-4 text-xs text-slate-400 font-bold uppercase tracking-widest">Or register manually</span>
          <div className="flex-grow border-t border-slate-600/50"></div>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-slate-400 font-medium">
          ...
          <button onClick={() => router.push('/login')} className="font-bold text-white hover:text-amber-500 transition-colors ml-1 border-b border-amber-500/30 pb-0.5">Sign in here</button>
        </p>
      </div>
    </div>
  );
